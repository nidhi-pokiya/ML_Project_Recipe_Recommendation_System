...
def recommend():
    ...
    # Process user input: split by comma.
    ...
    for dist, idx in zip(distances, indices):
        ...
        # Check for exact matches (an ingredient exactly in user input).
        exact_matches = [ing for ing in normalized_recipe_ings if ing in user_set]
        # If no exact match, try for a substring match.
        ...
        # Use the model's distance to compute similarity.
    ...
    # If any candidate is an exact match, filter to keep only exact matches.
    if any(cand["is_exact"] for cand in candidates):
        candidates = [cand for cand in candidates if cand["is_exact"]]
    
    # Sort candidates by descending similarity and then by fewer missing ingredients.
...
if __name__ == "__main__":
    try:
        if os.path.exists("recipe_recommender_model.pkl"):
            ...
            logger.error("recipe_recommender_model.pkl not found. Loading dummy model.")
            class DummyModel:
                def kneighbors(self, X, n_neighbors=5):
                    # Return dummy distances (1.0) and indices (0) for all neighbors.
                    return (np.ones((1, n_neighbors)), np.zeros((1, n_neighbors), dtype=int))
            recipe_model = DummyModel()
